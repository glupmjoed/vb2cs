...
link:https://www.nuget.org/packages/ICSharpCode.CodeConverter/[a .NET package
...
that `CodeConverter` itself depends heavily on the `Microsoft.CodeAnalysis`
(Roslyn) package, which must also be obtained
link:https://www.nuget.org/packages/Microsoft.CodeAnalysis/[from a package] or
link:https://github.com/dotnet/roslyn[from source].
...
(`/usr/share/dotnet`) and compiles the project for the target platform. It should
complete with the message "Build succeeded.", along with some statistics.
...
includes all dependencies in a local subdirectory] when building the project.
